...
```{r, warning = FALSE}
sdg3_sdg4_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "Distributions of SDG 3 Scores and SDG 4 Scores by Region",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       color = "Region")

ggplotly(sdg3_sdg4_scatterplot)
```
...
mytext <- paste(
    ...
    "Goal 10 Score: ", round(sdr_data_world_joined$goal_10_score, 2), 
    sep="") %>%
