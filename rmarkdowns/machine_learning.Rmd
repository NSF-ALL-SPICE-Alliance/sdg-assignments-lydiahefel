...
Random Forest Regression - SDG 10
```{r}
rf_sdg10_palma <- randomForest(normalized_score_sdg10_palma ~ ., data = sdr_data_imputed, importance = TRUE)
```
...
Now we can look at the variables most important in making the predictions
```{r}
importance_df2 <- as.data.frame(rf_sdg10_palma$importance)
```

Let’s take the top 10 and graph them
```{r}
importance_df_top_10_2 <- importance_df2 %>%
  rownames_to_column(var = "variable") %>%
  slice_max(n = 10, order_by = `%IncMSE`)
```
...
ggplot(importance_df_top_10_2, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  labs(title = "Most Important Variables in Predicting Maternal Mortality",
       ...
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
Lastly, we want to know how these important variables influence the prediction
```{r}
pdp::partial(rf_sdg10_palma, pred.var = "normalized_score_sdg10_gini", plot = TRUE)
```
Now that we saw how one variable influences the models prediction, how about 2 variables?
```{r}
pd2 <- pdp::partial(rf_sdg10_palma, pred.var = c("normalized_score_sdg10_gini", "normalized_score_sdg16_u5reg"))
# Default PDP
plotPartial(pd2)
```
