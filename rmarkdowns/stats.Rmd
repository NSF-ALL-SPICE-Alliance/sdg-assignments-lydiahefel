...
```{r, warning=FALSE, message=FALSE}
sdg10_sdg5_scatterplot <- ggplot(sdr_data, aes(x = goal_10_score, y = goal_5_score, label = country)) +
  geom_point() +
  geom_smooth() +
  theme_minimal() +
  labs(title = "Distributions of SDG 10 Scores and SDG 5 Scores by Country", x = "SDG 10 Score", y = "SDG 5 Score")

ggplotly(sdg10_sdg5_scatterplot)
```
...
Preface:
Sustainable Development Goal 10 is Reduced Inequalities and Sustainable Development Goal 5 is Gender Equality.
        
Interpretation:
There is a very weak positive correlation between SDG 10 and SDG 5, which gives the idea that SDG 10 and SDG 5 are less dependent upon one another. This correlation proves that the inequalities within countries are less likely to be gender-based. Due to the independence of the two goals, there are many points that do not follow the trend. Most notably, Yemen is the greatest outlier with an SDG 10 score of 66.3 and an SDG 5 score of 13.1. Yemen has the lowest SDG 5 score, and is the most apparent outlier, but Namibia has the lowest SDG 10 score of 5.5 (this excludes South Africaʻs SDG 10 score of 0.0 due to limited information). There are little to no countries that scored very low in both goals, but quite a few countries that scored very high in both goals, which makes sense because those countries to have higher overall scores.